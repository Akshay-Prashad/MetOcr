"""
Qwen2.5-VL-7B-Instruct, 4-bit quantized, sized to run on an 8GB RTX 4060.

Setup (run once):
    pip install -U transformers accelerate bitsandbytes qwen-vl-utils pillow

Notes on 8GB VRAM:
  - Use 4-bit NF4 quantization (bitsandbytes) -- fp16 7B will NOT fit
    alongside image tokens on 8GB.
  - Keep input crops small (row-bands, not full pages) -- Qwen2.5-VL's
